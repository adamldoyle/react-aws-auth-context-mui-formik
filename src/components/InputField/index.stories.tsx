...
const Template: Story<AllProps> = ({ defaultValue, required, onSubmit, ...args }) => {
  ...
  const Schema = Yup.object({
    [args.field]: required ? field.required(`${args.label} is required`) : field,
  });

  const formDefaults = Schema.getDefault();
  return (
    <Formik initialValues={formDefaults} validationSchema={Schema} onSubmit={((values) => onSubmit(values)) as any}>
      {() => (
        <Form>
          <Box>
            <InputField {...args} />
